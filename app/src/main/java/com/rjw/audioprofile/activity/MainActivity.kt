...
class MainActivity : AudioActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        for(profile in 0 until AudioProfileList.NO_PROFILES) {
            ...
            mRadioProfile[profile]!!.getButtonDrawable()!!.setColorFilter(configColour, PorterDuff.Mode.SRC_ATOP)
    ...
    companion object {
        ...
        const val CHANNEL_ID = "AudioProfileChannelId"
        val CHANNEL_NAME: CharSequence = "AudioProfile"
        const val CHANNEL_DESCRIPTION = "AudioProfile"
        ...
        fun createNotificationChannel(context: Context?) {
            try {
                val nm = mThis!!.getSystemService(NOTIFICATION_SERVICE) as NotificationManager
                if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                ...
                Alerts.toast("Creating notification channel: " + e.message)
            }
        }
        ...
        fun showServiceNotification(service: Service, msg: String?, pendingIntent: PendingIntent?) {
            if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                try {
                    ...
                    Alerts.toast("Creating notification: " + e.message)
                }
                Log.d(TAG, msg!!)
            }
        }
