...
class HSVColourWheel : View {
    ...
    private lateinit var mScaledPixels: IntArray
    ...
    private fun init() {
        ...
        mPointerPaint.setStrokeWidth(density * POINTER_LINE_WIDTH_DP)
    ...
    override fun onDraw(canvas: Canvas) {
        if(mBitmap != null) {
            ...
            mSelectedPoint.x = mRect!!.left + (-Math.cos(hueInPiInterval.toDouble()) * mColourHsv[1] * mInnerCircleRadius + mFullCircleRadius).toInt()
            mSelectedPoint.y = mRect!!.top + (-Math.sin(hueInPiInterval.toDouble()) * mColourHsv[1] * mInnerCircleRadius + mFullCircleRadius).toInt()
            canvas.drawLine(
    ...
    private fun createBitmap() {
        ...
        for(i in mScaledPixels.indices) {
            ...
            if(centerDist <= mScaledFullCircleRadius) {
                ...
                val alpha: Int
                alpha = if(centerDist <= mScaledInnerCircleRadius) {
                    255
                } else {
     ...
     */
    fun getColourForPoint(x: Int, y: Int, hsv: FloatArray): Int {
        val pointX = x - mFullCircleRadius
        val pointY = y - mFullCircleRadius
        val centerDist = Math.sqrt((pointX * pointX + pointY * pointY).toDouble())
        hsv[0] = (Math.atan2(pointY.toDouble(), pointX.toDouble()) / Math.PI * 180f).toFloat() + 180
        hsv[1] = Math.max(0f, Math.min(1f, (centerDist / mInnerCircleRadius).toFloat()))
